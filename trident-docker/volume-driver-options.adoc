...
| `encryption`  a| 
In questo modo, NetApp Volume Encryption (NVE) verrà attivato sul nuovo volume, per impostazione predefinita a. `false`. NVE deve essere concesso in licenza e abilitato sul cluster per utilizzare questa opzione.
...
== Cloud Volumes Service (CVS) sulle opzioni del volume AWS

Le opzioni di creazione dei volumi per il CVS sul driver AWS includono quanto segue:
...
| `snapshotReserve`  a| 
`questa opzione imposta la riserva di snapshot sulla percentuale desiderata. Il valore predefinito è NO, ovvero CVS selezionerà la riserva di snapshot (di solito 0%).
...
----
* Crea un volume premium da 500 GiB:
+
[listing]
----
